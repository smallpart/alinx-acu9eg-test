...
static uint16_t range_boundary[6] = {3000,4170,4500,6250,9000,12500};
...
static int check_dac_clk_freq_range(ad917x_handle_t *h, uint64_t dac_clk_freq_hz)
{
	...
	if (adi_chip_id.prod_id == AD9171_ID) {
		if ((dac_clk_freq_hz > DAC_9171_CLK_FREQ_MAX_HZ) || (dac_clk_freq_hz < DAC_CLK_FREQ_MIN_HZ)) {
			return API_ERROR_INVALID_PARAM;
		}
	} else {
		if ((dac_clk_freq_hz > DAC_CLK_FREQ_MAX_HZ) || (dac_clk_freq_hz < DAC_CLK_FREQ_MIN_HZ)) {
			return API_ERROR_INVALID_PARAM;
		}
	}
	return API_ERROR_OK;
}
static int spi_configure(ad917x_handle_t *h)
{
	switch (h->sdo) {
	case SPI_SDO:
...
static int sysref_configure(ad917x_handle_t *h)
{
	uint8_t tmp_reg;
	int err;
...
static int syncoutb_configure(ad917x_handle_t *h)
{	   
...
static int dac_init_sequence(ad917x_handle_t *h)
{
	int err;
	uint8_t tmp_reg = 0x0;
...
ADI_API int ad917x_get_revision(ad917x_handle_t *h, uint8_t *rev_major,
...
ADI_API int ad917x_set_dac_clk_freq(ad917x_handle_t *h, 
...
ADI_API int ad917x_get_dac_clk_freq(ad917x_handle_t *h, 
									uint64_t *dac_clk_freq_hz)
{	
	if (h == INVALID_POINTER) {
		return API_ERROR_INVALID_HANDLE_PTR;
...
ADI_API int ad917x_get_dac_clk_status(ad917x_handle_t *h,
				uint8_t *pll_lock_stat, uint8_t *dll_lock_stat)
{
	...
	if (pll_lock_stat != INVALID_POINTER) {
	...
	if (dll_lock_stat != INVALID_POINTER) {
...
ADI_API int ad917x_set_dac_clk(ad917x_handle_t *h,
...
ADI_API int ad917x_set_page_idx(ad917x_handle_t *h, const unsigned int dac, const unsigned int channel)
{
	...
	return ad917x_register_write(h, AD917X_SPI_PAGEINDX_REG, (uint8_t)((dac << 6) | channel));
}
...
ADI_API int ad917x_get_page_idx(ad917x_handle_t *h, int *dac, int *channel)
{
	...
	if ((dac == INVALID_POINTER) || (channel == INVALID_POINTER)) {
		return API_ERROR_INVALID_PARAM;
	}
